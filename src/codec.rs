...
pub fn compress(
    ...
    let q = if quality >= 100 { 1u16 } else { ((100u16.saturating_sub(quality as u16)) / 10 + 1).max(1) };

    // 1. Pad the image using clamp padding and convert to our Pixel struct
    let mut padded_pixels = vec![Pixel { channels: [0, 0, 0], count: channels }; w_pad * h_pad];
    for y in 0..h_pad {
        ...
        for x in 0..w_pad {
            ...
            if channels == 1 {
                padded_pixels[y * w_pad + x] = Pixel::new_gray(data[idx]);
            } else {
                let r = data[idx * 3];
                let g = data[idx * 3 + 1];
                let b = data[idx * 3 + 2];
                ...
                let y_val = g;
                let u_val = r.wrapping_sub(g);
                let v_val = b.wrapping_sub(g);
    ...
    for by in 0..rows {
        for bx in 0..cols {
            ...
            let mut block = vec![Pixel { channels: [0, 0, 0], count: channels }; b * b];
            for y in 0..b {
                for x in 0..b {
            ...
            let mut prev = Pixel { channels: [0, 0, 0], count: channels };
    ...
    // Header
    bitstream.extend_from_slice(b"MLZ2");
...
pub fn decompress(bitstream: &[u8]) -> Result<(u32, u32, u8, Vec<u8>)> {
    ...
    if &bitstream[0..4] != b"MLZ2" {
        return Err(anyhow!("Invalid magic bytes: must be MLZ2"));
    }
    ...
    let table_residual_1 = if channels == 3 {
        Some(FreqTable::deserialize(bitstream, &mut offset, 256)?)
    } else {
        None
    };
    let table_residual_2 = if channels == 3 {
        Some(FreqTable::deserialize(bitstream, &mut offset, 256)?)
    } else {
        None
    };
    ...
    let row_results: Result<Vec<Vec<Pixel>>> = (0..rows)
        ...
        .map(|by| {
            ...
            let mut row_pixels = vec![Pixel { channels: [0, 0, 0], count: channels }; cols * b * b];
                    ...
                    if cmd == 0 {
                        ...
                        for _ in 0..match_len {
                            if block_pixels.len() >= offset_idx {
                                ...
                                block_pixels.push(Pixel { channels: [0, 0, 0], count: channels });
                            }
                        }
    ...
    for y in 0..(height as usize) {
        for x in 0..(width as usize) {
            ...
            if channels == 1 {
                ...
                let u_val = pix.channels[1];
                let v_val = pix.channels[2];
                // Inverse Green-decorrelation:
                // G = Y, R = U + G, B = V + G
                let g = y_val;
                let r = u_val.wrapping_add(g);
                let b = v_val.wrapping_add(g);
